...
BEGIN
    ...
    if (@occurence0 >= 1)
    begin
        select @j0 = [dbo].[aMRC_posByCountBin](@bytes, @occurence0); -- смещение дл€ группы настроек є2 (4 байта)
    end
    else
    begin
        set @j0 = 2;
    ...
    -- теперь определ€ем смещение дл€ группы настроек є3 (оттуда возьмем байт, определ€ющий тип табличного буфера Table / View / Map)
    set @internalOffset = 0;
    set @byte1 = SUBSTRING(@bytes, @j0 + 2, 1); -- 1-й байт из настроек є2
    set @byte2 = SUBSTRING(@bytes, @j0 + 3, 1); -- 2-й байт из настроек є2
    set @byte3 = SUBSTRING(@bytes, @j0 + 4, 1); -- 3-й байт из настроек є2
    set @byte4 = SUBSTRING(@bytes, @j0 + 5, 1); -- 4-й байт из настроек є2
    ...
    begin
        set @j = @j0 + 4; -- это начало "0000" дл€ случа€ когда не заполнено ни одно из значение Title1, Title2, Extends
    end
    ...
    begin
        set @tableTypeOffset = @tableTypeOffset + 2; -- добавили смещение в 2 байта (в нем хранитс€ число DeleteActions)
    end
